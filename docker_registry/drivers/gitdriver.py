...
class Storage(file.Storage):
    ...
    def put_content(self, path, content):
        ...
        logprint.info("put_content at "+ path+ " "+ str(content)[:150]+"... v"+version,"RED")
...
class gitRepo():    
    ...
    # Parameter:
    # path -- path to layer visible outside of driver (something like: images/a9cde43434aef/layer)
    def getLayerPath(self, path):
        parts = os.path.split(path)
        basename = parts[1]
        if (basename != "layer"):
            logprint.error("Not a layer path in layer storage block (_init_path): "+str(path))
            return path
    ...
    def createCommit(self):
        ...
        self.branch_name = self.makeBranchName()        
        logprint.info("Creating commit for" +self.imageID[:8] + " branch:" + str(self.branch_name)+" parent:" + str(self.parentID)[:8],"IMPORTANT")
    ...
    def storeLayer(self):
    	logprint.functionStart("storeLayer")
        global working_dir, layer_dir
    ...
    def untar(self, source=None, dst=None):
    	logprint.functionStart("untar",source,dst)
    ...
    def makeCommit(self, comment="Comment"):
        logprint.functionStart("makeCommit",comment)
        try:
            self.gitcom.add("-A")
    ...
    def checkoutBranch(self, branch_name, reset=None):
    	logprint.functionStart("checkoutBranch",branch_name, reset)
        global working_dir, layer_dir
    ...
    def checkoutImage(self, imageID):
    	logprint.functionStart("checkoutImage",imageID)
        self.checkoutCommit(self.getCommitID(imageID))

    def checkoutCommit(self,commitID):
    	logprint.functionStart("checkoutCommit",commitID)
        #global working_dir
    ...
    def makeBranchName(self,tag=None):
    	logprint.functionStart("makeBranchName",tag);
    ...
    def getBranchName(self, commitID):
    	logprint.functionStart("getBranchName",commitID);
        if commitID is None:
    ...
    def newBranch(self,branch_name,commitID=None):        
    	logprint.functionStart("newBranch",branch_name,commitID)
    ...
    def prepareCheckout(self, path):
        global working_dir
        logprint.functionStart("prepareCheckout",path)
    ...
    def prepareLayerTar(self,path):
        ...
        logprint.functionStart("prepareLayerTar",path)
        ...
        tar_path = self.getLayerPath(path)
    ...
    def cleanDir(self, dir=None):
        logprint.functionStart("cleanDir",dir)
        ignore=(".git")
        global working_dir
        if dir is None:
            dir = working_dir
    ...
    def checked_commit(self):
        global working_dir
        logprint.functionStart("checked_commit")
